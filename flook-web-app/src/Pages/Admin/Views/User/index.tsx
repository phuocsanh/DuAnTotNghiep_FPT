...
import { DataGrid, GridColumns, GridRowsProp, gridPageCountSelector,
  gridPageSelector,
  useGridApiContext,
...
const UserTable: React.FC<TableProps> = props => {
  ...
  const ListUser = Selector.auth.DataManyUser()
  
  array?.forEach((item: any, i:number) => (item.id = i + 1));
  console.log('array table', array);

  const dispatch = useDispatch();

  const row: GridRowsProp = [
    {id: 1, userName: '123',password: '123', displayName: '123', phoneNumber: '123', email: '123', roles: 'Admin'}
  ]
  ...
  return (
    <Box sx={{ height: '100%', width: '100%', backgroundColor: 'white', borderRadius:'10px' }}>
      <DataGrid
        rows={row}
...
const CustomButton = (props: {Component?:any | undefined, title?: string| undefined, openNote?: boolean| false, handleTooltipOpen?: any| undefined, handleTooltipClose?: any| undefined}) => {
  const { Component, title, openNote, handleTooltipOpen, handleTooltipClose } = props
  ...
  return (
    <Tooltip 
      ...
      title="Chọn nhiều hơn 1 item để delete many"
    >
      <ButtonBase disabled sx={{
          borderRadius: '16px', 
          padding: '10px',
          margin: '10px',
...
function CustomFooter(props:any) {
  ...
  const handleTooltipOpen = function() {
    setOpenNote(props.selectionRow.length >= 0 ? true : false)
  }
  ...
  return (
    <Grid container sx={{borderTop: '1px solid rgba(224, 224, 224, 1)' }}>
      <Grid item xs={12} sm={6} sx={{alignSelf: 'center'}}>
        <CustomButton title='Create User' Component={<AddCircleOutlineOutlinedIcon sx={{mr:1}} fontSize='small'/>}/>
