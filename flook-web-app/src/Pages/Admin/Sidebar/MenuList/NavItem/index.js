...
const NavItem = ({ item, level }) => {
  ...
  let listItemProps = {
    component: forwardRef((props, ref) => <Link ref={ref} {...props} to={`${namePage.admin}${item.url}`} target={itemTarget} />),
  };
  if (item?.external) {
  ...
  const itemHandler = (id) => {
    dispatch({ type: actionTypes.onOffDrawer, id });
    if (matchesSM) dispatch({ type: '@customization/SET_MENU', opened: false });
  };
  ...
  useEffect(() => {
    ...
    if (currentIndex > -1) {
      dispatch({ type: actionTypes.onOffDrawer, id: item.id });
    }
